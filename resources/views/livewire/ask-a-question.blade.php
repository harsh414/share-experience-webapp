    ...
    <div class="text-center px-6 py-2 pt-6"  x-data="{flashMessage:true}">
        @if(session()->has('question_success'))
            <div class="bg-indigo-900 text-center py-4 lg:px-4 mb-4" x-show="flashMessage">
                <div class="p-2 bg-indigo-800 items-center text-indigo-100 leading-none lg:rounded-full flex lg:inline-flex" role="alert">
                    <span class="font-semibold mr-2 text-left flex-auto">{{session()->get('question_success')}}</span>
        ...
        @if(session()->has('qerror'))
            <div class="bg-indigo-500 text-center py-4 lg:px-4 mb-4" x-show="flashMessage">
                ...
                <span class="text-white font-semibold mr-2 text-left flex-auto">{{session()->get('qerror')}}</span>
        ...
        <form action="{{route('ask-question')}}" method="POST" class="px-3 py-6">
            {{@csrf_field()}}
            <div>
            ...
            <div>
                <input type="text" name="company_name" id="company_name_2" class="bg-white text-sm border-none w-full text-sm rounded mt-4 placeholder-gray-500" placeholder="Company Name" >
            </div>
            <div class="mt-4">
                <textarea name="question" id="question" cols="30" rows="16" class="w-full bg-gray-100 rounded-xl border-none placeholder-gray-900 text-sm px-4 py-2" placeholder="Ask a question :)"></textarea>
            </div>
